...
func (r *retryBuffer) post(buf []byte, query string, auth string, location string) (*responseData, error) {
...
func (r *retryBuffer) get(buf []byte, query string, auth string, location string) (*responseData, error) {
	if atomic.LoadInt32(&r.buffering) == 0 {
		resp, err := r.p.post(buf, query, auth, "")
		// TODO A 5xx caused by the point data could cause the relay to buffer forever
		if err == nil && resp.StatusCode/100 != 5 {
